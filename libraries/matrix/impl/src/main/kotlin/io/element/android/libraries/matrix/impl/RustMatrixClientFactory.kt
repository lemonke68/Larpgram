        ...
        return clientBuilderProvider.provide()
            ...
            .roomKeyRecipientStrategy(
                strategy = if (featureFlagService.isFeatureEnabled(FeatureFlags.OnlySignedDeviceIsolationMode)) {
                    ...
                    // Правка форка: ALL_DEVICES — ключи получают все устройства аккаунта получателя,
                    // как в обычном Element и как ждут люди от TG («вошёл — значит это ты», решение
                    // юзера 2026-09-24). Раньше стоял ONLY_TRUSTED_DEVICES: он отдаёт ключи чужим
                    // устройствам, только если отправитель вручную подтвердил получателя. Никто этого
                    // не делает, поэтому сообщения не расшифровывались на новых сессиях получателя
                    // (случай toluze → lemonke67, 2026-09-25). ERROR_ON_VERIFIED_USER_PROBLEM не
                    // берём: он блокирует отправку целиком (см. историю правки 2026-08-18).
